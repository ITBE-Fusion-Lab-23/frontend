...
function AboutUs() {
    ...
    const scrollToProfile = (profile_name) => {
        if (profileRefs.current[profile_name] && profileRefs.current[profile_name].current) {
            profileRefs.current[profile_name].current.scrollIntoView({ behavior: 'smooth', block: 'start' });
    ...
    const ProfileEntry = React.forwardRef(({ imgSrc, profile_name, bio ,linkedinUrl}, ref) => {
        return (
            <div ref={ref} className="profile-entry">
                ...
                <div className="text-and-button">
                    <h3>{profile_name}</h3>
                    <p>{bio}</p>
                    <a href={linkedinUrl}  className="linkedin-button" target="_blank" onClick={(e) => { console.log(typeof(linkedinUrl)+linkedinUrl); }} >LINKEDIN</a>
                </div>
